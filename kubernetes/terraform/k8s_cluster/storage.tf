...
resource "aws_ebs_volume" "ceph_storage_rancher" {
  ...
  availability_zone = aws_instance.k8s_rancher[count.index].availability_zone
}
...
resource "aws_volume_attachment" "ceph_storage_rancher_ec2" {
  count = var.k8s_rancher_count

  device_name = "/dev/sdf"
  volume_id   = aws_ebs_volume.ceph_storage_rancher[count.index].id
  instance_id = aws_instance.k8s_rancher[count.index].id
}