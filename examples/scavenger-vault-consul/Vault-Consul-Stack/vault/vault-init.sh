...
# create the secret to retrieve
vault kv put secret/scavenger file_path=scavenger/answer/answer.txt
...
cat policies/scavenger-policy.json | vault policy write app -
vault policy read app

# create the app token
vault token create -policy=app | awk '/token/ {
if ($1 == "token") print "scavenger_Token="$2
else if ($1 == "token_accessor") print "scavenger_Token_Accessor="$2
}' \
| tee >> .env
...
echo $scavenger_Token|vault login -
